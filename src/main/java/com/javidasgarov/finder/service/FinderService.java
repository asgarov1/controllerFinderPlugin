...
import com.javidasgarov.finder.util.UrlUtil;
...
public class FinderService {
    ...
    private static final List<String> SPRING_REQUEST_MAPPING_ANNOTATIONS = List.of(
            "@RequestMapping",
            "@GetMapping",
            "@PostMapping",
            "@PatchMapping",
    ...
    public static List<String> getPrefixes(PsiAnnotation prefixAnnotation) {
        return Stream.of(prefixAnnotation)
                ...
                .map(attributeValue -> getAnnotationValues(attributeValue.getText()))
                .findFirst()
    ...
    private static boolean isControllerAnnotation(PsiAnnotation psiAnnotation) {
        ...
        return SPRING_REQUEST_MAPPING_ANNOTATIONS.contains(annotationName);
    }
    ...
    public static Optional<PsiAnnotation> findMatchingAnnotation(Optional<PsiAnnotation> prefixAnnotation,
        ...
        return annotationUrls.entrySet().stream()
                .filter(entry -> UrlUtil.containsMatch(entry.getValue(), searchUrl))
    ...
    private static List<String> generateUrls(PsiAnnotation annotation, List<String> prefixes) {
        ...
        if (attributeValue.isPresent()) {
            List<String> values = getAnnotationValues(attributeValue.get().getText());
